...
This document describes Hermes's trust model, names the one
...
security vulnerabilities. **Hermes does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Hermes access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Hermes's documentation
...
Hermes supports this in two ways:

- **Hermes's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Hermes's in-process heuristics
...
A malicious or buggy plugin is not a vulnerability in Hermes
itself. Bugs in Hermes's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Hermes does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Hermes's own documentation, or reasonable
  operator expectations would predict — including cases where
  Hermes has documented a stance about how its output should
...
- Hermes includes supply-chain guards for MCP server

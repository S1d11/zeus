...
This document describes Zeus's trust model, names the one
...
security vulnerabilities. **Zeus does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Zeus access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Zeus's documentation
...
Zeus supports two OS-level isolation postures. They address
...
Zeus supports this in two ways:

- **Zeus's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Zeus's in-process heuristics
...
Zeus filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Zeus
itself. Bugs in Zeus's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Zeus does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Zeus's own documentation, or reasonable
  operator expectations would predict — including cases where
  Zeus has documented a stance about how its output should
...
- Zeus includes supply-chain guards for MCP server

"""
Interactive setup wizard for Zeus.
...
_DOCS_BASE = "https://github.com/S1d11/zeus"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Zeus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Zeus using environment variables or config commands:")
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Zeus runs shell commands and code.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Zeus to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print_info("   https://github.com/S1d11/zeus")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Zeus from anywhere.")
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Zeus config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Zeus.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Zeus's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Zeus can preview what would be imported before making any changes.")
    print()
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Zeus (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Zeus Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,

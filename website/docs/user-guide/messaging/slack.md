...
sidebar_position: 4
title: "Slack"
description: "Set up Hermes as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Hermes to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
4. Enter an app name (e.g., "Hermes") and select your workspace
5. Click **Create App**
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Hermes what time is it?`). In channels, Hermes replies in a thread attached to that message. |
...
Make sure the bot has been **invited to the channel** (`/invite @Hermes`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Hermes` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Hermes` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Hermes`)

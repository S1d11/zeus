...
description: Control your smart home with Zeus via Home Assistant integration.
...
Zeus integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Zeus"
5. Copy the token
...
Zeus registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Zeus".

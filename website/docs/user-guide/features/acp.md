...
description: "Use Hermes inside ACP-compatible editors such as VS Code, Zed, and JetBrains"
---

# ACP Editor Integration

Hermes can run as an ACP server, letting ACP-compatible editors talk to Hermes over stdio and render:
...
2. Select **Hermes** from the built-in agent list.
...
- In Zed, open the ACP Registry with `zed: acp registry` and search for **Hermes**.
...
Install `uv` from the official uv installation docs, then retry the Hermes thread from Zed.

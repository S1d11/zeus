...
description: "Use Zeus inside ACP-compatible editors such as VS Code, Zed, and JetBrains"
---

# ACP Editor Integration

Zeus can run as an ACP server, letting ACP-compatible editors talk to Hermes over stdio and render:
...
2. Select **Zeus** from the built-in agent list.
...
- In Zed, open the ACP Registry with `zed: acp registry` and search for **Zeus**.
...
Install `uv` from the official uv installation docs, then retry the Zeus thread from Zed.

...
Zeus 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。
...
| `retain_context` | `conversation between Zeus and the User` | 保留记忆的上下文标签 |

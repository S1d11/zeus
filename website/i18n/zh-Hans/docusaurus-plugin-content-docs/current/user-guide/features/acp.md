...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 Zeus"
---

# ACP 编辑器集成

Zeus 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 Hermes 通信并渲染：
...
- 在 Zed 中，使用 `zed: acp registry` 打开 ACP Registry 并搜索 **Zeus**。
...
从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 Zeus 线程。

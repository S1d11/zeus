...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str):
        ...
        self.proc = subprocess.Popen(
            ...
            bufsize=1,
            cwd=os.getcwd(),
            # slash_worker runs the Zeus → needs provider credentials.
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s HERMES_HOME around a profile-scoped RPC handler.

    Profile-scoped RPCs need config/state to resolve to the focused profile even
    in app-global remote mode, where one backend serves every profile. No-op for
    the launch profile (own-profile backends already resolve it).
    """
...
def _(rid, params: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="pdf_attach_") as td:
        ...
        try:
            res = subprocess.run(argv, capture_output=True, text=True, timeout=120, stdin=subprocess.DEVNULL, creationflags=windows_hide_flags())
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Zeus Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Zeus/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Zeus Desktop. If it serves Zeus/Desktop UI or another unrelated app, stop that process and report failure.",
